      ...
      value: function _handleRecurringEvents() {
        ...
        recurringEvents.forEach(function (item) {
          ...
          var newEvents = all.map(function (time, index) {
            return _extends({}, item, {
              ...
              start: rule.origOptions.tzid
                ? _this2.localeMoment
                    .utc(time)
                    .utcOffset(_this2.localeMoment().utcOffset(), true)
                    .format(_index.DATETIME_FORMAT)
                : _this2.localeMoment(time).format(_index.DATETIME_FORMAT),
              end: rule.origOptions.tzid
                ? _this2.localeMoment
                    .utc(time)
                    .utcOffset(_this2.localeMoment().utcOffset(), true)
                    .add(oldEnd.diff(oldStart), 'ms')
                    .add(_this2.localeMoment(oldUntil).utcOffset() - _this2.localeMoment(item.start).utcOffset(), 'm')
                    .format(_index.DATETIME_FORMAT)
                : _this2.localeMoment(time).add(oldEnd.diff(oldStart), 'ms').format(_index.DATETIME_FORMAT),
            });
          });
          newEvents.forEach(function (newEvent) {
            ...
            if (
              _this2.isEventInTimeWindow(eventStart, eventEnd, windowStart, windowEnd) &&
    ...
    },
    {
      ...
      value: function _resolveDate(num) {
        ...
        if (this.viewType === _index.ViewTypes.Week) {
          this.startDate =
            date != undefined
              ? this.localeMoment(date).startOf('week').format(_index.DATE_FORMAT)
              : this.localeMoment(this.startDate).add(num, 'weeks').format(_index.DATE_FORMAT);
          this.endDate = this.localeMoment(this.startDate).endOf('week').format(_index.DATE_FORMAT);
        } else if (this.viewType === _index.ViewTypes.Day) {
          this.startDate =
            date != undefined
              ? this.selectDate
              : this.localeMoment(this.startDate).add(num, 'days').format(_index.DATE_FORMAT);
          this.endDate = this.startDate;
        } else if (this.viewType === _index.ViewTypes.Month) {
          this.startDate =
            date != undefined
              ? this.localeMoment(date).startOf('month').format(_index.DATE_FORMAT)
              : this.localeMoment(this.startDate).add(num, 'months').format(_index.DATE_FORMAT);
          this.endDate = this.localeMoment(this.startDate).endOf('month').format(_index.DATE_FORMAT);
        } else if (this.viewType === _index.ViewTypes.Quarter) {
          this.startDate =
            date != undefined
              ? this.localeMoment(date).startOf('quarter').format(_index.DATE_FORMAT)
              : this.localeMoment(this.startDate).add(num, 'quarters').format(_index.DATE_FORMAT);
          this.endDate = this.localeMoment(this.startDate).endOf('quarter').format(_index.DATE_FORMAT);
        } else if (this.viewType === _index.ViewTypes.Year) {
          this.startDate =
            date != undefined
              ? this.localeMoment(date).startOf('year').format(_index.DATE_FORMAT)
              : this.localeMoment(this.startDate).add(num, 'years').format(_index.DATE_FORMAT);
          this.endDate = this.localeMoment(this.startDate).endOf('year').format(_index.DATE_FORMAT);
        } else if (
      ...
      value: function _createInitHeaderEvents(header) {
        ...
        var endValue = this.showAgenda
          ? this.viewType === _index.ViewTypes.Week
            ...
            : this.localeMoment(this.endDate).add(1, 'days').format(_index.DATETIME_FORMAT)
          : this.cellUnit === _index.CellUnits.Hour
          ? start.add(this.config.minuteStep, 'minutes').format(_index.DATETIME_FORMAT)
          : start.add(1, 'days').format(_index.DATETIME_FORMAT);
        return {
          time: header.time,
        ...
        this.events.forEach(function (item) {
          ...
          if (resourceEventsList.length > 0) {
            ...
            resourceEvents.headerItems.forEach(function (header, index) {
              ...
              if (
                _moment.unix(headerEnd) >= _moment.unix(eventStart) &&
                _moment.unix(headerStart) < _moment.unix(eventEnd)
              ) {
                ...
                if (header.count > resourceEvents.rowMaxCount) {
                  ...
                  var newRowHeight =
                    ...
                    (_this5.config.creatable && _this5.config.checkConflict === false ? 20 : 2);
            ...
            if (hasSummary) {
              var rowsCount =
                cellMaxEventsCount <= cellMaxEventsCountValue && resourceEvents.rowMaxCount > cellMaxEventsCount
                  ...
                  : resourceEvents.rowMaxCount;
              var newRowHeight =
                (rowsCount + 1) * _this5.config.eventItemLineHeight +
                (_this5.config.creatable && _this5.config.checkConflict === false ? 20 : 2);
              if (newRowHeight > resourceEvents.rowHeight) resourceEvents.rowHeight = newRowHeight;
            }
          });
